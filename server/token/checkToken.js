...
module.exports = async ( ctx, next ) => {
    ...
    // "token eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyX2lkIjoiMTIzNDU2NyIsImlhdCI6MTUxNTMzODA1MiwiZXhwIjoxNTE1MzM4MTEyfQ.UEn-w2scInve2sMSev_-tiDqxhfGP9rxFNMHjbC9l9w"
    const token = authorization.split(' ')[1];
    let tokenContent;
    try {
        tokenContent = await jwt.verify(token, 'test');     //如果token过期或验证失败，将抛出错误
    } catch (err) {
